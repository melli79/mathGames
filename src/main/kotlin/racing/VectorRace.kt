...
class VectorRace(val numPlayers :UInt) : KeyBoardComponent() {
    ...
    private fun evolve() {
        ...
        (0..< numPlayers.toInt()).sortedByDescending { poss[it] }.forEach { i ->
            ...
            positions[i] = when {
                crashWith!=null -> handleCarCarCrash(i, v, crashWith, newPos)
                abs(coords.off) > w -> handleWallCrash(coords, i)
    ...
    private fun handleCarCarCrash(i :Int, v :Vector2D,
    ...
    private fun handleWallCrash(pos :Position, i :Int) :Point2D {
        println(pos)
    ...
    override fun paint(g :Graphics) {
        ...
        if (font==null)
            font = g.font.deriveFont(Font.BOLD, 48.0f)
        g.drawRoute(route)
    ...
    private fun paintStatus(g :Graphics) {
        val p = positions[turn]
        g.color = Color.BLACK
        g.drawString(
            "Step $step, ${names[turn]}'s turn: @%.2f km, %.1f km/h".format(
                route.computeDistanceTraveled(poss[turn]) / 10,
                v0 * sqrt(velocities[turn].norm2())
            ),
            5, 20
        )
        val winners = winners.mapIndexed { pos, player -> "${describePosition(pos)}: ${names[player]}" }
        when {
    ...
    private fun paintRanking(g :Graphics) {
        var y = height/2 -25*numPlayers.toInt()
        g.font = font
        winners.forEachIndexed { pos, player ->
            y += 55
            g.color = colors[player % colors.size]
            g.drawString("${describePosition(pos)}: ${names[player]}", width/2 -130, y)
        }
    }
    ...
    private fun Rect.computeScale(width :Int, height :Int) :Rect {
        val dx = min(width/dx, height/dy)
        return Rect(x0 -(width/dx-this.dx)/2, y1 +(height/dx-dy)/2, dx, -dx)
    }
